...
local gameServerName = "MiniSBGame"
...
_StartReconnet = function()
    ...
    log("迷你骰宝 _StartReconnet------------------- " .. NetworkManager.reconnectCount)
    if NetworkManager.reconnectCount > reconnectTryTimes then
        NetworkManager.reconnectCount = nil
        _CloseLoading(NetworkManager)
        MiniSBNotification.GamePost("NetworkClose")
        return
    end
...
_OnOpen = function()
    ...
    MiniSBNotification.GamePost("CreateSocketSuccess")
end
...
_ShowLoading = function(self)
    if not self.loadingFlag then
        -- log("_ShowLoading---------------------1111111111111111111111111111")
        MiniSBNotification.GamePost("ShowConnecting", 1)
        self.loadingFlag = true
    end
end

_CloseLoading = function(self)
    -- log("_CloseLoading---------------------222222222222222222222222")
    self.loadingFlag = false
    MiniSBNotification.GamePost("HideConnecting")
end
