...
local TrailIOSHallView = CC.uu.ClassView("TrailIOSHallView",nil,HallView)
local viewCtrClass = require("View/TrailView/TrailIOSHallViewCtr")

function TrailIOSHallView:OnCreate()
...
function TrailIOSHallView:InitUI()
...
function TrailIOSHallView:InitGameList()
	local classname = "TrailIOSGameList"
	self.gameList = CC.uu.CreateHallView(classname,self:FindChild("Panel/GameList"),self,self.transform)
end

function TrailIOSHallView:AddClickEvent()
	--打开邮箱
	...
	self:AddClick("Panel/DownBG/MoreBtn/MoreBG/settingBtn/btn",function ()
		self.viewCtr.gameDataMgr.SetSwitchClick("TrailIOSSetUpSoundView")
		CC.ViewManager.Open("TrailIOSSetUpSoundView")
	end)
end

function TrailIOSHallView:OnFocusIn()
...
--检查后续弹窗
function TrailIOSHallView:CheckBehindView()
...
function TrailIOSHallView:RefreshMainRedDot(param)

end

function TrailIOSHallView:RefreshSubRedDot(param)

end

function TrailIOSHallView:RefreshWaterSprinklingBtnStatus(isShow,isOpen)
	
end

function TrailIOSHallView:InitTextByLanguage()

end

function TrailIOSHallView:RefreshChatPri(priState)
	self:FindChild("Panel/ChatPanel/ChatBtn"):SetActive(not priState)
end

--筹码达到10w，游客玩家提示
function TrailIOSHallView:ChipChange()

end

return TrailIOSHallView