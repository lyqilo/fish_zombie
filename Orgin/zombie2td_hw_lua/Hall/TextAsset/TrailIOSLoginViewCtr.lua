...
local TrailIOSLoginViewCtr = CC.class2("TrailIOSLoginViewCtr")

local AsynCall = require("Common/AsynCall")

function TrailIOSLoginViewCtr:ctor(view, loginType)
	self:InitVar(view,loginType)
end

function TrailIOSLoginViewCtr:InitVar(view,loginType)
...
function TrailIOSLoginViewCtr:OnCreate()
...
function TrailIOSLoginViewCtr:PreloadAssets()
...
--获取游戏列表数据
function TrailIOSLoginViewCtr:ReqGameInfo()
...
function TrailIOSLoginViewCtr:StartUpdate()
	UpdateBeat:Add(self.Update,self);
end

function TrailIOSLoginViewCtr:StopUpdate()
	UpdateBeat:Remove(self.Update,self);
end

function TrailIOSLoginViewCtr:Update()
...
function TrailIOSLoginViewCtr:RegisterEvent()
	--大厅服ip获取通知
...
function TrailIOSLoginViewCtr:UnRegisterEvent()
...
function TrailIOSLoginViewCtr:OnRequestVersionInfo()
	--获取强制更新版本号信息
	local reqFunc;
	reqFunc = function()
...
function TrailIOSLoginViewCtr:OnOpenSocket()
	--连接socket
	CC.Network.Start();
end

function TrailIOSLoginViewCtr:OnSocketConnect()
...
function TrailIOSLoginViewCtr:OnSocketStop()
...
--退出游戏
function TrailIOSLoginViewCtr:SafeQuit()
...
function TrailIOSLoginViewCtr:ReadyToEnter()
	...
	if self.gameData.IsInit() and self.isOpenSocket then
		...
		elseif curLoginWay == self.loginDefine.LoginWay.OPPO then
			...
			self:OnOppoLogin()
		elseif curLoginWay == self.loginDefine.LoginWay.Apple then
			self:OnAppleLogin()
		else
...
function TrailIOSLoginViewCtr:OnGuestLogin()
	--游客登录
	self:SetBtnState(false);
	...
	CC.Request("Login",data,function(err,result)
			CC.uu.Log(result,"Request.Login success-->>result:")
		...
		function(err)
			CC.uu.Log(err,"Request.Login fail:")
			--玩家被封号
...
function TrailIOSLoginViewCtr:OnAppleLogin()
	--苹果登录
	self:SetBtnState(false);
	self.view:RefreshUI({showPercent = true, percent = 0});
	...
	local callback = function(result)
		...
		-- local data = {}
		-- data.code = 0
		-- data.bid = "com.lwf.Taiwa"
		-- data.identityToken = "eyJraWQiOiJmaDZCczhDIiwiYWxnIjoiUlMyNTYifQ..iiJD0Kl_bMtw9aH_1ZlQxfGvPX7EGTPtcZLmpND5xghgM_STio21JaqyIU-QUq_YV9tV9frYSW-GNnzNlk_asnfuNs1psm8zBimxqtYc3sMXZbpFCDDdsN2eZLPK1zgw5NtPfdv7_NwiaVRPlUHxMCZC-ILWGe9LGTpSqS4hq6fQD_hFBaWEXugmKW-QlfGwrkNBqSHEV4oZlENlj5VHaT1GB_dox8cB577_PH9EB0OZfaKZsz_SczYnCITlhMlIn8iXWi3uLJQmwCCn2zLmhvSXSBKQF_VoFeYPUQYogL17yM5UJtPIunpqIyL0558MocPedrQcsRr77VuAXIGlsA"
		-- data.authorizationCode = "cfcfb25413de2413da95f83678cbc9a34.0.rwrw.uXDb2Y4Swi4E1_ho_g_Q4Q"
		-- data.isAutoAppleLogin = true

		if data.code == 0 then
			local appleResult = {};
			appleResult.UserIdentifier = data.userID;
			appleResult.IdentityToken = data.identityToken;
			appleResult.AuthorizationCode = data.authorizationCode;
			appleResult.IsAutoAppleLogin = data.isAutoAppleLogin
			appleResult.GuestUsr,appleResult.GuestPwd = self:GetThirdPartyLoginUserInfo();
			CC.uu.Log(data.isAutoAppleLogin,"data.isAutoAppleLogin:")
			CC.LocalGameData.SetLocalStateToKey("AppleLoginBid",data.bid)
			CC.Request("AppleLogin",appleResult,function(err,result)
					CC.uu.Log(result,"Request.AppleLogin success:")
					CC.Player.Inst():SetLoginInfo(result)
					--游客登录置空第三方登录token
					CC.Player.Inst():SetThirdPartyToken();
					self:LoginWithProcess(result, nil, self.loginDefine.LoginWay.Apple, CC.shared_enums_pb.RT_IOS)
					self.networkRspCount = self.networkRspCount + 1;
				end,
				function(err)
					CC.uu.Log("Request.AppleLogin fail")
					--玩家被封号
					if err == CC.shared_en_pb.PlayerIsClosed then
						self:SetBtnState(true);
					else
...
function TrailIOSLoginViewCtr:Register()
...
function TrailIOSLoginViewCtr:GetThirdPartyLoginUserInfo()
...
function TrailIOSLoginViewCtr:OnLineLogin()
	--Line登录
	self:SetBtnState(false);
	self.view:RefreshUI({showPercent = true, percent = 0});
...
function TrailIOSLoginViewCtr:OnFacebookLogin()
	--Fackbook登录
	self:SetBtnState(false);
	self.view:RefreshUI({showPercent = true, percent = 0});
...
function TrailIOSLoginViewCtr:OnOppoLogin()
	--Oppo登录
	self:SetBtnState(false);
	self.view:RefreshUI({showPercent = true, percent = 0});
...
function TrailIOSLoginViewCtr:LoginWithProcess(data, partnerToken, loginType, registerType)
...
function TrailIOSLoginViewCtr:AysnRequest(param, succCb, errCb)
...
function TrailIOSLoginViewCtr:LoginWithToken(data,partnerToken,loginType,registerType)
...
function TrailIOSLoginViewCtr:LoadPlayerWithPropType(loginData)
	--先获取玩家个人信息数据
	local req = {};
...
function TrailIOSLoginViewCtr:RegToPublisher(logindata)
	local req = {};
...
function TrailIOSLoginViewCtr:GetNewPlayerFlag()
...
function TrailIOSLoginViewCtr:GetNoviceSignInState()
	local req = {};
...
function TrailIOSLoginViewCtr:GetPlayerTaskState()
	local req = {};
...
function TrailIOSLoginViewCtr:CheckMeIfAgent()
...
function TrailIOSLoginViewCtr:CheckBirthday(logindata)
	local req = {};
...
function TrailIOSLoginViewCtr:ReqLoadPlayerGameInfo(logindata)
...
function TrailIOSLoginViewCtr:EnterGame()
	CC.uu.DelayRun(0.5, function()
		CC.ViewManager.Replace("HallView")
	end);
end

function TrailIOSLoginViewCtr:SaveRegister(registerType)
	--保存注册信息到APPSFLYER
	local tips
...
function TrailIOSLoginViewCtr:SetBtnState(flag)
...
function TrailIOSLoginViewCtr:OnOpenService()
...
function TrailIOSLoginViewCtr:OnAddNodeCount(nodeIndex)
	if CC.DebugDefine.GetDebugMode() then return end
	...
	if self.clickCount[1] + self.clickCount[2] >= 20 then
		CC.uu.Log("===Debug Open====")
		CC.DebugDefine.SetDebugMode(true);
	end
end

function TrailIOSLoginViewCtr:GetOSEnum()
	return CC.Platform.GetOSEnum()
end

function TrailIOSLoginViewCtr:Destroy()
...
function TrailIOSLoginViewCtr:SetLocalGameVersion()
