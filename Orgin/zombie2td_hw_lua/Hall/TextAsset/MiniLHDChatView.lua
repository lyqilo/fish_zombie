local CC = require("CC")
local GC = require("GC")
local MiniLHDChatView = CC.uu.ClassView("MiniLHDChatView")
...
function MiniLHDChatView:ctor(param)
...
function MiniLHDChatView:OnCreate()
...
function MiniLHDChatView:registerEvent()
	MiniLHDNotification.GameRegister(self, PLAYER_CHAT_MSG, onMsg)
end

function MiniLHDChatView:unRegisterEvent()
...
-- ActionIn 正常打开动作
-- ************************************************************
function MiniLHDChatView:ActionIn()
...
-- ActionOut 正常关闭动作
-- ************************************************************
function MiniLHDChatView:ActionOut()
	if Moving then
		return
	end
...
-- @moving : 是否移动中
-- ************************************************************
function MiniLHDChatView:setMoveing(moving)
	Moving = moving
...
-- hideBlockBtn 隐藏空白按钮
-- ************************************************************
function MiniLHDChatView:hideBlockBtn()
	self.blockBtn:SetActive(false)
end

function MiniLHDChatView:changeInputFieldPlaceholder()
...
function MiniLHDChatView:createHisChat(resp, isFirst)
	...
	if isFirst then
		item.obj:SetAsFirstSibling()
		table.insert(ContParent, 1, tPre)
	else
		table.insert(ContParent, tPre)
	end
end

function MiniLHDChatView:initItem(tran, data)
...
function MiniLHDChatView:OnDestroy()
...
function MiniLHDChatView:queryChatHistory()
	query(self)
end

--缓冲池相关
function MiniLHDChatView:getChatItem(name, parent)
	-- log(CC.uu.Dump(chatItemList, "chatItemList =", 10))
...
function MiniLHDChatView:RemoveChatItem(name, item)
	if item then
			...
			if self.sendTime == 0 then
				...
				if not allSpace(text) and utf8len(text) > 0 then
					if utf8len(text) <= INPUT_LIMIT then
						...
						local errStr = MNLHDConfig.LOCAL_CHAT[MNLHDConfig.GameLanguage]["MessageLimit"]
						CC.ViewManager.ShowTip(errStr)
					end
				else
					local errStr = MNLHDConfig.LOCAL_CHAT[MNLHDConfig.GameLanguage]["EmptyMessage"]
					CC.ViewManager.ShowTip(errStr)
				end
			end
		end
