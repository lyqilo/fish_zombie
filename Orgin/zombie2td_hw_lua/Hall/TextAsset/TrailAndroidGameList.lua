-- local CC = require("CC")
-- local GameList = require("View/HallView/GameList")
-- local TrailAndroidGameList = CC.uu.ClassView("TrailAndroidGameList",nil,GameList)
local viewCtrClass = require("View/TrailView/TrailAndroidGameListCtr")

local CC = require("CC")
local TrailAndroidGameList = CC.uu.ClassView("TrailAndroidGameList")

local TripleJackpot = {

}

function TrailAndroidGameList:ctor(transform,hallview,obj)
...
function TrailAndroidGameList:Create()
	self:OnCreate()
end

function TrailAndroidGameList:OnCreate()
...
function TrailAndroidGameList:InitGameList(param)
	local index = 1
...
function TrailAndroidGameList:CreRCPrefab(param,parent,index)
	local obj = nil
	local preName = param.name
	obj = CC.uu.LoadHallPrefab("prefab",preName,parent)
	obj.transform.localScale = Vector3(0,0,1)
	param.obj = obj
	local effect = obj.transform:FindChild("icon/effect")
	if effect then
...
function TrailAndroidGameList:CreMiniPrefab(param,parent,index)
	local obj = nil
	local preName = param.name
...
function TrailAndroidGameList:InitPrefab(param,index)
	local id = param.id
...
function TrailAndroidGameList:InitMatchInfo(id)
...
function TrailAndroidGameList:GetTimestamp(Time)
...
function TrailAndroidGameList:OnClickCard(id)
	if id == 4001 then
		self:EnterLot()
		return
	elseif id == 4002 then
...
function TrailAndroidGameList:EnterLot()
...
function TrailAndroidGameList:GameUnlockGift(id)
...
function TrailAndroidGameList:VipChanged(level)
...
function TrailAndroidGameList:DownloadProcess(data)
...
function TrailAndroidGameList:DownloadFail(id)
	if self.gameList[id] == nil then return end
	local obj = self.gameList[id].obj
	obj:FindChild("obj/icon"):SetActive(true)
...
function TrailAndroidGameList:StartUpdate()
	UpdateBeat:Add(self.Update,self);
end

function TrailAndroidGameList:StopUpdate()
	UpdateBeat:Remove(self.Update,self);
end

function TrailAndroidGameList:Update()
...
function TrailAndroidGameList:RefreshMatch(value,param)
	local obj = value.obj
...
function TrailAndroidGameList:GetCurrentShowTime(param,jackpot)
	local showText = nil
	local bMatch = false
	local physical = nil
	local currentTime = os.time()
...
function TrailAndroidGameList:InitGameJackpots(bState)
...
function TrailAndroidGameList:UpdateGoldPool(param,id)
	local jackpotNum = 0
	if self:JudgeJackpotState(id) then
...
function TrailAndroidGameList:RollGoldPool(param,id)
...
--合并奖池
function TrailAndroidGameList:JudgeJackpotState(id)
	local state = false
...
--获取合并奖池
function TrailAndroidGameList:GetTripleJackpot()
	local count = 0
	for i = 1, #TripleJackpot do
...
function TrailAndroidGameList:SetCanClick(flag)
	self._canClick = flag
end

function TrailAndroidGameList:OnDestroy()
